...
import { StudentSchema, studentSchema, teacherSchema, TeacherSchema, UserSchema } from "./auth.schema";
...
import { sendPasswordResetEmail, sendVerificationEmail } from "../../utils/mailHandlers";
...
const login = async (req: Request, res: Response, next: NextFunction) => {
  try {
   ...
   let profileId = foundUser.teacherProfile?.id ?? foundUser.studentProfile?.id ?? '';
   if(profileId === ''){
...
const resetPassword = async (req: Request, res: Response,next: NextFunction) => {
  try {
    ...
    if (user == null) {
      ...
      ResponseHandler.send(res,200, {userId: user.id}, "successfully sent password reset email");
...
const resendOtp = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    ResponseHandler.send(res, 201, {otp, ...foundUser}, "successfully resent otp");
  } catch (err) {
    next(err);
  }
};
